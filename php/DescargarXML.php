...
<?php
    ...
    else
    {
        ...
        $datosEmisor.="   <ambiente>1</ambiente>\r";
        ...
        $sqlFacturasXML= "select a.IdMov as IdMov,a.Fecha as fechaEmision,'$xDireccion' as dirEstablecimiento, 
        				      ...
        				      concat(e.Apellidos,' ',e.Nombres) as razonSocialComprador,
        				      ...
        				      'DOLAR' as moneda,Total as total, e.Email,a.Total as Total,e.IdTipoDocumento as IdTipoDoc,a.FactSri as SecSRI
						from movta a, clientes e
                        where a.Fecha >='$fechaInicio' and a.Fecha<='$fechaFin'
                         and a.IdCliente = e.IdCliente";
        ...
        while($regFacturasXML= mysqli_fetch_array($rsFacturasXML))
        {
           ...
           $strFacturasXML.="    <secuencial>".str_pad($IdMov,9,'0',STR_PAD_LEFT)."</secuencial>\r";
           ...
           $strFacturasXML.="   <totalDescuento>0</totalDescuento>\r";
           ...
           {
              if($regFacturaDetalle['ValorImpuesto']==0)
              {
              ...
              #-------------------------------------------------------------------------------------------------------
               ...
               $strFacturasXML.="         <baseImponible>".number_format($xSubTotalBase0,2)."</baseImponible>\r";
               $strFacturasXML.="         <valor>0</valor>\r";
               $strFacturasXML.="     </totalImpuesto>\r"; 
               $strFacturasXML.="     <totalImpuesto>\r";     
               $strFacturasXML.="         <codigo>2</codigo>\r";
               $strFacturasXML.="         <codigoPorcentaje>2</codigoPorcentaje>\r";
               $strFacturasXML.="         <baseImponible>".number_format($xSubTotalBaseIva,2)."</baseImponible>\r";
               $strFacturasXML.="         <valor>".number_format($xValorIvaBaseIva,2)."</valor>\r";
               $strFacturasXML.="     </totalImpuesto>\r";
          
               $strFacturasXML.="   </totalConImpuestos>\r";
               $strFacturasXML.="   <propina>0</propina>\r";
               $strFacturasXML.="   <importeTotal>".number_format($regFacturasXML['Total'],2)."</importeTotal>\r";
           ...
           while($regFormasPago= mysqli_fetch_array($rsFormaPago))
           {
              ...
              $strFacturasXML.="         <total>".number_format($regFormasPago['valorFormaPago'],2)."</total>\r";
              $strFacturasXML.="      </pago>\r";                        
           }
           $strFacturasXML.="   </pagos>\r";
           $strFacturasXML.="</infoFactura>\r";
           ...
           $strSQLDetalle       = "select movref.IdReferencia as codigoPrincipal,CargaIva as cargaIva,
           								  ...
           								  movref.Descuento as descuento,ValorImpto as valorImpuesto,
           ...
           while($regDetalleXML  =   mysqli_fetch_array($rsDetalle))
           {
                  ...
                  $xTarifaImpuesto    = $xTarifaImpuesto;
           	    ...
           	    else
           	    {
                    ...
                    $strFacturasXML.="    <precioUnitario>".number_format($regDetalleXML['precioUnitario'],2)."</precioUnitario>\r";
                    $strFacturasXML.="    <precioSinSubsidio>0</precioSinSubsidio>\r";
                    $strFacturasXML.="    <descuento>0</descuento>\r";
                    $strFacturasXML.="    <precioTotalSinImpuesto>".number_format($regDetalleXML['precioTotalSinImpuesto'],2)."</precioTotalSinImpuesto>\r";
                    $strFacturasXML.="    <impuestos>\r";
                        $strFacturasXML.="        <impuesto>\r";
                            ...
                            $strFacturasXML.="        <tarifa>".number_format($xTarifaImpuesto,2)."</tarifa>\r";
                            $strFacturasXML.="        <baseImponible>".number_format($xBaseImponible,2)."</baseImponible>\r";
                            $strFacturasXML.="        <valor>".number_format($xValorImpuesto,2)."</valor>\r";
                        $strFacturasXML.="        </impuesto>\r";
                    $strFacturasXML.="    </impuestos>\r";
                $strFacturasXML.="    </detalle>\r";
           }
           ...
           $strFacturasXML.="     <campoAdicional nombre=\"Email:\">xsaltos@hotmail.com</campoAdicional>\r";
           $strFacturasXML.="</infoAdicional>\r";
