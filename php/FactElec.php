  ...
  <form id='formulario' name='formulario'  role='form' method='POST' enctype='multipart/form-data'  action='DescargarXML.php'>  
      <div class='container'>
        <div class='panel panel-default'>
            <div class="panel-heading" style="font-size: 20px;color:#0080FF;font-weight:bold;">GENERACION DE ARCHIVOS XML PARA LA FACTURACION ELECTRONICA
      ...
      var fechaFin = document.getElementById('FechaFin').value;
        ...
        $.ajax({
                  ...
                  success: function(data)
                  {
                   ...
                   {
                     contador++;
                     html='<tr><td>'+data.data[i].Fecha+'</td>'+
                          '<td>'+data.data[i].IdMov+'</td>'+
                          '<td>'+data.data[i].SubTotal+'</td>'+
                          '<td>'+data.data[i].Descuento+'</td>'+
                          '<td>'+data.data[i].Impuesto+'</td>'+
                          '<td>'+data.data[i].Total+'</td>'+
                          "<td><a href='#' onClick=popup_detalle_ventas('"+data.data[i].Refe+"') ><span title='Detalle del Prodcuto'><img src='../img/detalle.png'></span></a></td></tr> ";
