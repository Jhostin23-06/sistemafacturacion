...
<?php
    ...
    else
    {
        ...
        $sqlSecSRI      = "select * from srisecuencial where IdAlmacen=$xIdAlmacen and IdTipoDocumento=2 ";
        ...
        if($conexion->query($sqlInsert)==TRUE)
        {
          ...
          for($i=0;$i<=$NumeroItems;$i++)
          {
                ...
                #-----------------Tabla Stock ------------------------#
                $sqlStock     = "select * from stock where IdAlmacen=$xIdAlmacen AND IdBodega=$xIdAlmacen AND IdReferencia=$xReferencias[$i]";
                $rsStock      = mysqli_query($conexion,$sqlStock);
                $regStock     = mysqli_fetch_assoc($rsStock);                
                $stock        = $regStock['stock']-$xCantidades[$i];
                $sqlInsertDetalle   = "INSERT INTO movref VALUES($xIdAlmacen,'$TipDoc',$xIdCaja,$IdMov,$j,$xReferencias[$i],$xCantidades[$i],$xPrecios[$i],$xTotalBrutoLinea[$i],$xDescuentoLinea[$i],$xSubtotalLinea[$i],$xIvaLinea,$xTotalLinea[$i],$IvaActual,0,$xCostoVenta)";
                    ...
                    if($conexion->query($sqlInsertDetalle)==TRUE)
                    {
                        ...
                        #---------Graba en histrx -----------------------------
                        $sqlInsertHistrx= "INSERT INTO histrx VALUES('0',now(),$xReferencias[$i],$xIdAlmacen,$xIdAlmacen,$IdMov,'NV',$xCantidades[$i],$xCostoVenta,$xIdAlmacen,$xCostoVenta)";
                        if($conexion->query($sqlInsertHistrx)==TRUE)
                        {
                           $sqlUpdateSecSRI="UPDATE srisecuencial SET SecuencialSRI=$xSriSecuencial+1,IdMovInterno=$IdMov  where IdTipoDocumento=2 and IdAlmacen=$xIdAlmacen";
                ...
                switch ($xFormasPago[$i])
                {
                  ...
                  case '4':   #---
                       $sqlInsertFormasPago="INSERT INTO formapagofactura(IdAlmacen,TipDoc,IdMovta,IdSecuencial,IdFormaPago,
                                        ...
                                        VALUES($xIdAlmacen,'$TipDoc',$IdMov,$j,$xFormasPago[$i],$xBancos[$i],
                       ...
                       $sqlInsertFormasPago="INSERT INTO formapagofactura(IdAlmacen,TipDoc,IdMovta,IdSecuencial,IdFormaPago,
                        ...
                        $sqlInsertCtasxCobrar="INSERT INTO ctasxcobrar(
                          IdMov,TipDoc,IdEmpresa,IdCliente,IdAlmacen,FechaMov,ValorFactura,Estado,aud_usuario_proc,aud_fecha_proc)
                                        VALUES(
                          $IdMov,'$TipDoc',$xIdEmpresa,$xIdCliente,$xIdAlmacen,now(),$xTotalFactura,'P','$xUsuario',now())";
          ...
          $sqlInsertNumerosSerie=" INSERT INTO ventasitemsseries SELECT $IdMov,IdAlmacen,IdReferencia,NumeroSerie,UserName,DATE_SUB(NOW(), INTERVAL 5 HOUR) from tmp_numeros_serie".
        ...
        { 
          $flagError=1;
          $msgError.= $xSriSecuencial.' '.$sqlSecSRI." Error al grabar movta de Pago: .<br>". $sqlInsert. '  '.$conexion->error;
        }
        if($msgError==null)
        {
           $msgError=$IdMov ;
