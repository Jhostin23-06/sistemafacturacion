...
<html>
	<head>
		...
		<title> Búsqueda de Proveedpres</title>
	</head>
	...
	<body>
		<form id="form1" name="form1" method="GET" action="?">
			<strong style="font-family: 'Poppins', sans-serif;
                    ...
                    width: 100%;
                    font-size: 12px;color: #0040FF;" >Buscar por Nombres: </strong>
			...
			<table   border="1" cellspacing="0" cellpadding="0">
	          <style>
				...
				<tr  style='color: #0040FF;background-color:#FFFF00;'>
					...
					<td width="10%" style='padding: 5px;'><center>  <b>RUC - Cédula</b></center></td>
   			...
   			<?php
					...
					{ 
					    if($_GET['q']<>"")
					    {
							...
							$query = "SELECT * FROM proveedores WHERE DescripcionProveedor LIKE '%".$_GET['q']."%' 
										OR DescripcionProveedor LIKE '%".$_GET['q']."%' ORDER BY DescripcionProveedor";
				
							$r_query = mysqli_query($conexion,$query);
							while ($reg=mysqli_fetch_array($r_query) )
					        {
					           // $i++;
					        	$v_Id      	 	= $reg['IdProveedor'];
								$v_Cedula  	 	= $reg['RUC'];
					            $v_Nombres 	 	= $reg['DescripcionProveedor'];
					            $v_Telefonos 	= $reg['Telefonos'];
					            $v_Email 		= $reg['Email'];
					            $v_Direccion 	= $reg['Direccion'];
					            $v_Nombres   = str_replace(" ","&nbsp;", $v_Nombres);
					            $v_Nombres   = str_replace(" ","&nbsp;", $v_Direccion);
					            //$id=$row{'id'};
					            echo "<tr OnMouseOver='Resaltar_On(this);' 
					                      OnMouseOut='Resaltar_Off(this);' 
					                      OnClick=datos_proveedor($v_Id,'$v_Cedula','$v_Nombres','$v_Telefonos','$v_Email','$Direccion')> 
